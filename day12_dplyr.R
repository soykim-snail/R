...
exam %>% summarise()
exam %>% summarise(n())
exam %>% summarise(snail = n())
exam %>% tally() # 축약된 summarise. 갯수만 센다.
...
# add_tally() 와 add_count(..) 도 있음 : 새로운 열로 추가함
...
sum(df$score)   # 합계 산출
# 결측치 있는 행 제거하기
mean(df$score, na.rm = T)  # na.rm=T : 결측치 제외
sum(df$score, na.rm = T)   # na.rm=T : 결측치 제외
...
boxplot(mpg$hwy) # 기본은 range = 1.5*IQR
boxplot(mpg$hwy, range = 1) # 이상치 range 조절 가능
View(ggplot2::mpg)
#상자그림 통계치 출력
