...
(chars <- letters[1:10]) #대입과 출력을 동시에 (...)
...
mat1[1,1]
mat1[2,];mat1[,3]
mat1[1,1,drop=F] #drop=FALSE 매트릭스 구조를 깨트리지 말아라.
...
class(score);str(score);mode(score)
summary(score)
...
btype <- factor(
  c("A", "O", "AB", "B", "O", "A"), #level 우선. 결측치로 인식
  levels=c("A", "B", "O"))
btype
summary(btype)
levels(btype)
...
table(score$result)  #모든타입의 데이터에 대해 빈도수 반환
...
emp[c(F,F,F,F,F,F,F,F,T,F,F,F,
      F,F,F,F,F,F,F,F),]
emp[emp$ename=="KING",]  #sql의 where 절과 동일한 효과
...
y >= z
y <= z
y == 50 # c(50, 50, 50, 50, 50) 즉, 자동으로 dim을 맞춰준다.
y > 50
