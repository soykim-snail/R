# [문제1] airquality 라는 데이터셋이 몇 개의 관측치를 가지고 있으먀 
# 어떠한 변수들을 가지고 있는지 채크하려 한다. 이 때 사용되는 R코드를 작성하시오.
str(airquality)
# 
...
df2
# 
# [문제4] c() 함수로 먼저 벡터를 생성한 다음 data.frame()사용해서 다음과 같이 
# 구성되는 데이터 프레임 df3를 만들어 출력해 본다.
# (제품명이 팩터형이 되지 않게 한다.)
# 제품명	가격	판매량
# 사과	1800	24
# 딸기	1500	38
...
가격 <- c(1800, 1500, 3000)
판매량 <- c(24, 38, 13)
df3 <- data.frame(제품명, 가격, 판매량, stringsAsFactor=F)
summary(df3)
# 
# [문제5] 앞에서 만든 데이터 프레임을 이용해서 
# 과일 가격 평균, 판매량 평균을 구하여 출력한다.
mean(df3$가격)
mean(df3$판매량)
# 
# [문제6] 다음 세 벡터를 이용하여 데이터프레임 df4를 생성하고, 
# name 변수는 문자, gender 변수는 팩터, math 변수는 숫자 데이터의 
# 유형이라는 것을 확인하시오.
...
# 100 이상 150 미만이면 B, 70 이상 100 미만이면 C, 
# 70 미만이면 D 등급을 부여하고 grade 변수에 저장하시오.	
...
emp$job <- factor(emp$job)
emp$depno <-factor(emp$deptno)
str(emp)
...
subset(emp, subset=(emp$job!="ANALYST"), select=c('ename','job','sal'))

# [문제14] emp 에서 업무가 SALESMAN 이거나 ANALYST 인 사원들의 이름, 직업을 출력한다.
emp[(emp$job=='SALESMAN' | emp$job=='ANALYST'), c('ename','job','sal')]
subset(emp, subset=(emp$job=='SALESMAN'|emp$job=='ANALYST'), select = c('ename','job','sal'))

# [문제15] emp 에서 커미션이 정해지지 않은 직원의 이름과 월급 정보를 출력한다.
emp[(is.na(emp$comm)), c('ename','job','sal')]
